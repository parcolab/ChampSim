...
public:
  ...
  typedef struct req
  {
    ...
    int champsim_cpu_id;
    // Note PACKET must not be a pointer
  ...
  void set_params(DRAMParams* dramParams) {
    param = dramParams;
  }
...
private:
  ...
  int refresh2(int channel);

  int precharge(int channel);
  int write(int channel);
  int write_rowhit(int channel);
  int activate(int channel);

  int activate_thread(int channel, int thread_id);
  int read(int channel);
  int read_rowhit(int channel);

  int read_thread(int channel, int thread_id);

  int read_rowhit_thread(int channel, int thread_id);

  void set_drain_writes_mode(int channel);
  int get_gainful_inst_thread(int channel);

  int get_gainful_thread(int channel);
  void set_gainful_thread();
  void init_gainful_thread_ptr(int channel);
  void init_gainful_inst_thread_ptr(int channel);
  void sort_gainful_thread();
  void update_lastread();
  void commit_lastread(int core, uint pc);
  void decrement_ght(int core, uint pc);
  void commit_ght(int core, uint pc);
  GhtEntry_t* spil_ght(int core);
  GhtEntry_t* get_ght(int core, uint pc);
  void init_gainful_thread();
  void init_ght();
  void init_lastread();
