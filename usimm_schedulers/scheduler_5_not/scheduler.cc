...
stat_entry* stat[stat_entry_num];

void stat_destory() {
	for (int i = 0; i < stat_entry_num; ++i) {
		stat_entry* e = stat[i];
		while (e) {
			if (e->key) {
				free(e->key);
...
void stat_init() {
	stat_destory();
	for (int i = 0; i < stat_entry_num; ++i) {
		stat[i] = malloc(sizeof(stat_entry));
		stat[i]->key = NULL;
		stat[i]->counter = 0;
		stat[i]->next = NULL;
	}
}
uint32_t stat_hash(const char*key) {
	uint32_t h = 0;
	for (int i = 0; i < strlen(key); ++i) {
		h += key[i];
	}
	return h % stat_entry_num;
}
stat_entry* stat_find(const char*key) {
	stat_entry* e = stat[stat_hash(key)];
	while (e) {
...
stat_entry* stat_add(const char*key) {
	stat_entry* e = stat[stat_hash(key)];
	while (e) {
		if (!e->key) {
		...
		if (!e->next) {
			e->next = malloc(sizeof(stat_entry));
		}
		e = e->next;
	}
	assert(false);
...
bool stat_inc(const char*key) {
	stat_entry* e = stat_find(key);
	if (e) {
		e->counter++;
	} else {
		e = stat_add(key);
...
uint64_t stat_get(const char*key) {
	stat_entry* e = stat_find(key);
	if (e) {
		return e->counter;
	} else {
...
void stat_print() {
	...
	qsort(keys, n, sizeof(char *), comparestr);

	for (int i = 0; i < n; ++i) {
		printf("%s: %llu\n", keys[i], stat_get(keys[i]));
	}
}
...
bool is_prio_thread(int t) {
	if (thread_info[t].prio > MAX_PRIO - 8) {
		return true;
	}
...
void step(int ch) {
	if (ch == 0) {
		if (CYCLE_VAL % (NUMCORES * T_DATA_TRANS * 6) == 0) {
			forThread(t) {
